...
def __emptyroute():
    return {'RouteNo':None,'DirectionID':None,'Direction':None,'RouteHeading':None, 'Trips':None}
...
#    if len(inputlist) == 1:
#        _route_dict = inputlist
#        print("{} {} {} {}".format(_route_dict['RouteNo'], _route_dict['DirectionID'], _route_dict['Direction'], _route_dict['RouteHeading']))
#        if len(_route_dict) == 0:
#            return [__mergedicts(_route_dict, __emptytrip())]
#        #check for empty route dict
#        if _route_dict['RouteNo'] is None and _route_dict['DirectionID'] is None and _route_dict['Direction'] is None and _route_dict['RouteHeading'] is None:
#            _trip_dict = __emptytrip()
#            _route_trip_dict = __mergedicts(_route_dict, _trip_dict)
#            return [_route_trip_dict]
#        else:
#            return _route_dict
        
    for item in inputlist: # 1 to many
        try:
            if item['Trips'] is None:
                _route_dict = __emptyroute()
                _trip_dict = __emptytrip()
                _route_trip_dict = __mergedicts(_route_dict, _trip_dict)
                return _route_trip_dict
        except:
